...
final class TelegramWebhookController
{
    ...
    {
        if ($this->webhookSecret === '' || !hash_equals($this->webhookSecret, $request->getHeaderLine('X-Telegram-Bot-Api-Secret-Token'))) {
        ...
        if ($text === '/start' || $text === '/help') {
            $this->telegram->send($chatId, "TMS bot. Generate a one-time link command in TMS → Notifications, then send it here.");
            return $response->withStatus(200);
        }
        ...
        if (preg_match('/^\/link_(\d+)_([a-f0-9]{24}\.[a-f0-9]{64})$/D', $text, $matches) === 1) {
            ...
            if ($userId > 0 && $this->tokens->consume($userId, $matches[2], new DateTimeImmutable('now'))) {
                ...
                $this->telegram->send($chatId, 'TMS account linked. Telegram notifications are now available.');
            } else {
                $this->telegram->send($chatId, 'This TMS link command is invalid, expired, or already used.');
            }
        }
        return $response->withStatus(200);
    }
