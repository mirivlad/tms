(() => {
    ...
    const labels = {
        ...
        created_at: dialog.dataset.created || 'Created', last_activity_at: dialog.dataset.lastActivity || 'Last activity',
    };
    const yes = dialog.dataset.yes || 'Yes';
    const no = dialog.dataset.no || 'No';
    const noActivity = dialog.dataset.noActivity || 'No activity yet';
    const add = (key, value) => {
        const dt = document.createElement('dt'); const dd = document.createElement('dd');
        dt.textContent = labels[key] || key;
        dd.textContent = ['active','email_verified','approved'].includes(key) ? (value ? yes : no) : (key === 'last_activity_at' && !value ? noActivity : (value ?? '—'));
        body.append(dt, dd);
    };
    const open = async (id) => {
        ...
        try {
            ...
            for (const key of ['id','username','email','role','active','email_verified','approved','created_at','last_activity_at']) add(key,data[key]);
